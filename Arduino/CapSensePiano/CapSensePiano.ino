...
/*
  ...
  Originally by Tyler Crumpton and Nicholas Jones
  Edited by Dean Biskup and Julie Shen
...
#define LED_PIN      13   // The output pin for the piezo buzzer
#define NUM_OF_SAMPLES  10   // Higher number whens more delay but more consistent readings
#define CAP_THRESHOLD   215  // Capactive reading that triggers a note (adjust to fit your needs)
...
CapacitiveSensor keys[] = {CS(10), CS(9), CS(8), CS(7), CS(6), CS(5), CS(4), CS(3)};
...
void loop() {    
  ...
  bool flag = false;
  for (int i = 0; i < 8; ++i) {
    int value = keys[i].capacitiveSensor(NUM_OF_SAMPLES);
    // If the capacitance reading is greater than the threshold, play a note:
    if(value > CAP_THRESHOLD) {
      //tone(A0, notes[i], 175); // Plays the note corresponding to the key pressed
      digitalWrite(LED_PIN, HIGH);
      flag = true;

      // Prints out the key number to the Serial monitor
      Serial.print(i);
    }
  }
  if (flag == false) {
